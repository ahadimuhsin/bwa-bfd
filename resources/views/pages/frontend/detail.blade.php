                ...
                <div class="px-3 flex-none" style="width: 320px">
                    <div class="rounded-xl p-4 pb-8 relative bg-white">
                        <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                            <img src="{{ $item->galleries()->exists() ? Storage::url($item->galleries()->inRandomOrder()->first()->url) : 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==' }}" alt=""
                                class="w-full h-full object-cover object-center" />
                        </div>
                        <h5 class="text-lg font-semibold mt-4">{{ $item->name }}</h5>
                        <span class="">IDR {{ number_format($item->price, 0, '', '.') }}</span>
                        <a href="{{ route('detail', $item->slug) }}" class="stretched-link">
                            <!-- fake children -->
                        </a>
                    </div>
                </div>
